...
def render_p5_animation(E, V0, L, prob, img_data):
    ...
    function draw() {{
        ...
        noStroke();
        fill(249, 115, 22, 120); // オレンジ色（最後は透明度：0〜255）
        let bStartPixel = 320;   // 障壁の開始位置
        let bWidthPixel = {L} * 40; // 障壁の幅（Lに連動）
        rect(bStartPixel, 50, bWidthPixel, 200); // 壁を描画

        // パラメータ設定
        let bStart = 320; // 障壁の開始位置 (ピクセル)
        let bWidth = {L} * 40; // 障壁の幅
        let p = {prob};
        
        // 2. 波束（パルス）の計算と描画
        noFill();
        strokeWeight(3);
        stroke(34, 211, 238); // YOUと同じ水色
        ...
        for (let x = 0; x < width; x++) {{
            ...
            if (x_norm < 4.0) {{
                y = wave * 60; // 入射波
            }} else if (x_norm >= 4.0 && x_norm <= 4.0 + {L}) {{
                // 障壁内の減衰
                let d = map(x_norm, 4.0, 4.0 + {L}, 1.0, sqrt(p));
                y = wave * 60 * d;
            }} else {{
                y = wave * 60 * sqrt(p); // 透過波
            }}
            vertex(x, height/2 + y);
        }}
        endShape();
        ...
        if (t > 4.0) {{
            let t_ref = 4.0 - (t - 4.0);
            for (let x = 0; x < bStart; x++) {{
                let x_norm = map(x, 0, width, 0, 10);
                let ref_wave = exp(-pow(x_norm - t_ref, 2) / 0.5) * sin(2 * PI * 2 * x_norm);
                vertex(x, height/2 + ref_wave * 40 * sqrt(1-p));
            }}
        }}
        endShape();
        ...
        // 時間を進める（動画と同じ速度感）
        t += 0.05;
        if (t > 12.0) isAnimating = false; // 画面外に消えたら停止
    }}
    </script>
    """
    components.html(p5_code, height=350)
